...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const rentPaymentsResponse = await api.get('/api/admin/rent-payments');
      setRentPayments(rentPaymentsResponse.data);
      
      const maintenanceResponse = await api.get('/api/admin/maintenance-requests');
      setMaintenanceRequests(maintenanceResponse.data);
      
      const apartmentsResponse = await api.get('/api/admin/apartments');
      setApartments(apartmentsResponse.data);
    } catch (error) {
      console.error('Error fetching data:', error);
    }
  ...
  const handleSetRent = async () => {
    try {
      await api.post('/api/admin/rent', { rentAmount });
      setRentAmount('');
      // Optionally re-fetch rent payments if rent amount affects them
    } catch (error) {
      console.error('Error setting rent amount:', error);
    }
  };

  const handleApproveRentPayment = async (rentId, currentStatus) => {
    const newStatus = currentStatus === 'approved' ? 'unapproved' : 'approved';
    try {
      await api.put(`/api/admin/manage-rent/${rentId}`, { status: newStatus });
      fetchData(); // Re-fetch rent payments to update status
    } catch (error) {
      console.error('Error approving rent payment:', error);
    }
  };

  const handleDeleteRentPayment = async (rentId) => {
    try {
      await api.delete(`/api/admin/delete-rent/${rentId}`);
      fetchData(); // Re-fetch rent payments
    } catch (error) {
      console.error('Error deleting rent payment:', error);
    }
  };

  const handleMarkRequestAsDone = async (requestId) => {
    try {
      await api.put(`/api/admin/maintenance-requests/${requestId}`, { status: 'done' });
      fetchData(); // Re-fetch maintenance requests
    } catch (error) {
      console.error('Error marking request as done:', error);
    }
  ...
  const handleAddApartment = async () => {
    try {
      await api.post('/api/admin/apartment', apartmentDetails);
      setApartmentDetails({ number: '', details: '' });
      fetchData(); // Re-fetch apartments
    } catch (error) {
      console.error('Error adding apartment:', error);
  ...
  const handleUpdateApartment = async (id) => {
    try {
      await api.put(`/api/admin/apartment/${id}`, apartmentDetails);
      setApartmentDetails({ number: '', details: '' });
      fetchData(); // Re-fetch apartments
    } catch (error) {
      console.error('Error updating apartment:', error);
  ...
  const handleDeleteApartment = async (id) => {
    try {
      await api.delete(`/api/admin/apartment/${id}`);
      fetchData(); // Re-fetch apartments
    } catch (error) {
      console.error('Error deleting apartment:', error);
    }
  ...
  const handleAssignApartment = async () => {
    try {
      await api.put(`/api/admin/assign-apartment/${selectedTenant}`, { apartmentId: selectedApartment });
      setSelectedTenant('');
      setSelectedApartment('');
      fetchData(); // Re-fetch tenants and apartments
    } catch (error) {
  ...
  return (
    <Container className="mt-5">
      ...
      <Card className="mb-4">
        <Card.Header as="h5">All Rent Payments</Card.Header>
        <Card.Body>
          <Table striped bordered hover responsive>
            <thead>
              <tr>
            ...
            <tbody>
              {Array.isArray(rentPayments) && rentPayments.map((payment) => (
                <tr key={payment._id}>
                  ...
                  <td>
                    <Button
                      variant={payment.status === 'approved' ? 'warning' : 'success'}
                      onClick={() => handleApproveRentPayment(payment._id, payment.status)}
                      className="me-2"
                    >
                      {payment.status === 'approved' ? 'Unapprove' : 'Approve'}
                    </Button>
                    <Button
                      variant="danger"
                      onClick={() => handleDeleteRentPayment(payment._id)}
                    >
                      Delete
                    </Button>
                  </td>
      ...
      <Card className="mb-4">
        ...
        <Card.Body>
          {Array.isArray(maintenanceRequests) && maintenanceRequests.map((request) => (
            <div key={request._id}>
              <h6>{request.tenantName}</h6>
              <p>{request.message} - Status: {request.status}</p>
              {request.status === 'pending' && (
                <Button
                  variant="success"
                  onClick={() => handleMarkRequestAsDone(request._id)}
                >
                  Mark as Done
                </Button>
              )}
